...
# PANEL EXCLUSIVO PARA LA DESCRIPCIÓN DE LA CARTA
@onready var descripcion_carta: RichTextLabel = $vida/ColorRect2/DescripcionCartaLunar

# ELEMENTOS DE MEJORA Y VIDA (MANTIENEN SU RUTA ORIGINAL)
...
func _ready() -> void:
	...
	if descripcion_carta:
		contenedor_carta_ui = descripcion_carta.get_parent() # Esto obtiene a ColorRect2 de forma segura
		if contenedor_carta_ui:
			contenedor_carta_ui.hide() 
...
func _inicializar_tienda() -> void:
	...
	if not pool_disponible.is_empty():
		...
		if card_data_ofertada and sprite_luna:
			...
			# === AGRANDAR EL SPRITE DE FORMA LIMPIA ===
			var factor_escala: float = 1.0
			sprite_luna.scale = Vector2(factor_escala, factor_escala)
	else:
...
func _on_comprar_luna_pressed() -> void:
	if ruta_carta_ofertada == "" or card_data_ofertada == null:
		return
	...
	if game_manager.oro >= precio_luna:
		...
		var nombre_carta = card_data_ofertada.card_name if card_data_ofertada else "Carta"
		_mostrar_feedback("Adquirida: [color=cyan]" + nombre_carta.to_upper() + "[/color].")
...
func _on_comprar_mejora_pressed() -> void:
	if game_manager.oro >= precio_mejora:
		...
		_mostrar_feedback("Contenedor Expandido.\n[color=green](+20 HP Max)[/color]")
...
func _on_comprar_vida_pressed() -> void:
	...
	if game_manager.oro >= precio_vida:
		...
		_mostrar_feedback("Sistemas Reparados.\n[color=green](+25 HP)[/color]")
	else:
		_mostrar_feedback("[color=red]Oro insuficiente.[/color]")
